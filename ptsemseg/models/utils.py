...
class segnetDown_basic(nn.Module):
    def __init__(self, in_size, out_size,drop_out=None):
        super(segnetDown_basic, self).__init__()
        self.conv1 = conv2DBatchNormRelu(in_size, out_size, 7, 1, 3, bias=False)
...
class segnetUp_basic(nn.Module):
    def __init__(self, in_size, out_size,drop_out=None):
        super(segnetUp_basic, self).__init__()
        self.unpool = nn.MaxUnpool2d(2, 2)
        self.conv1 = conv2DBatchNorm(in_size, out_size, 7, 1, 3, bias=False)
        self.drop_out = drop_out
        if self.drop_out :
            #drop out layer
            self.drop_out = nn.Dropout(p=0.5)
